...
- fragment_states: list or array of fragment state vectors.
  ...
  - Units: positions in meters (m) or kilometers (km) and velocities in meters/second (m/s) or kilometers/second (km/s). Document and convert consistently before calling.
...
- units: string flag to indicate "m/s" or "km/s" so outputs are labeled correctly.
- thresholds: dict with numeric thresholds for class labels (e.g., low/medium/high ΔV in chosen velocity units).
...
Convergence accuracy reported as mean dispersion (converted to meters in the output)
